...
public:
    // 接收rosbag和lidar信息，imu的数值,来自imuPreintegration的odom数据；内存+参数初始化重置
    ImageProjection():
    deskewFlag(0)
    {
    ...
    // 点云输入，全部点云，提取点云空间重置（pointColInd与pointRange的定义问题）
    void allocateMemory()
    {
    ...
    // 激光数据处理：深度图生成，去畸变；深度图中提取点云，发布，重置参数
    ...
    // 将imuDeskewInfo数值用于每个点云点在一条线中的位置
