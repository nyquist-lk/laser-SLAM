...
cScanMatch::cScanMatch() {
  ...
  params.max_angular_correction_deg = 3.0; // 最大角度修正
  params.max_linear_correction = 0.05; // 最大直线修正
  params.max_iterations = 100; // 迭代中止次数
  params.epsilon_xy = 0.000001; // 迭代中止xy值
  params.epsilon_theta = 0.000001; // 迭代中止theta值
  params.max_correspondence_dist = 0.3;
  params.sigma = 0.010;
  params.use_corr_tricks = 1; // 控制两个函数
  params.restart = 0;
  params.restart_threshold_mean_error = 0.01;
  params.restart_dt = 1.0;
  params.restart_dtheta = 0.1; // 
  params.clustering_threshold = 0.25; // 聚类阈值
  params.orientation_neighbourhood = 40; // 同方向的邻域
  params.use_point_to_line_distance = 1; // 1 for PLICP, 0 for ICP
  ...
  params.laser[0] = 0; // 传感器相对于机器人坐标系的x
  params.laser[1] = 0; // 传感器相对于机器人坐标系的y
  params.laser[2] = 3.1415926; // 传感器相对于机器人坐标系的theta
  params.first_guess[0] = 0; // 初始估计x
  params.first_guess[1] = 0; // 初始估计y
  params.first_guess[2] = 0; // 初始估计theta
  params.min_reading = 0.5; // 最小激光范围
  params.max_reading = 20.0; // 最大激光范围
...
void cScanMatch::match(const std::vector<messageIO::laserScanData> &scan,
  ...
  for (int i = 1; i < ldp_size; i++)
  {
    ...
    last_odom.theta = normal_rad(results.x[2] + scan_odom.back().theta);
    ...
    // ROS_INFO("progress: %d", progress);
  }

  ROS_INFO("Invaild matches: %d", invalid_matches);
  ROS_INFO("Vaild rate: %f", (float)(1 - ((float)(invalid_matches) / (float)(ldp_size))));
